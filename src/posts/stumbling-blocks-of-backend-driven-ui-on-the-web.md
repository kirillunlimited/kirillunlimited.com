...
The disadvantage of smartphones is a very long release cycle that includes the approval stage by official mobile application marketplaces such as App Store and Play Market. As you know, there is no such problem on the web, as users get a new version as soon as the project is deployed. As a result, the BDUI concept along with the cross-platform approach introduces artificial obstacles that front-end developers have to overcome. It means one step forward for mobile and one step back for web. You should think carefully before agreeing to implement BDUI in your web project and see if the benefits really outweigh all the difficulties and disadvantages. Probably, it is a good idea to come up with a hybrid back-end service capable of producing different data sets: one for BDUI-based mobile platforms, and the other for the web, adhering to the classical "client-server" approach.